...
<div class="container">
	<div class='page-header'>
		<h1>Welcome to the THMIX Report!
			<small><?php
	...
	<a class='btn btn-primary' href='report_full.php' role='button'>View Full Report</a>
		...
		<table class="table table-condensed">
			...
			<?php
				...
				$sql = "SELECT id, device, trial, record, score, time, combo, perfect, great, good, miss, update_time
                    FROM tmx_devices ORDER BY update_time DESC LIMIT 10";
				$query = $db->prepare($sql);
				$query->execute();
		...
		<table class="table table-condensed">
			...
			<?php
				...
				$sql = "SELECT id, song, trial, record, score, time, combo, perfect, great, good, miss, update_time
                    FROM tmx_songs ORDER BY update_time DESC LIMIT 10";
				$query = $db->prepare($sql);
				$query->execute();
				while ($obj = $query->fetch()) {
					if ($obj->id == 1)
		...
		<table class="table table-condensed">
			...
			<?php
				...
				$sql = "SELECT song_id, device_id, trial, record, score, combo, perfect, great, good, miss, update_time
                    FROM tmx_records ORDER BY update_time DESC LIMIT 10";
				$query = $db->prepare($sql);
				$query->execute();
				while ($obj = $query->fetch()) {
		...
		<table class="table table-condensed">
			...
			<?php
				...
				$sql = "SELECT song_id, device_id, score, combo, perfect, great, good, miss, update_time
                    FROM tmx_trials ORDER BY update_time DESC LIMIT 10";
				$query = $db->prepare($sql);
				$query->execute();
				while ($obj = $query->fetch()) {
					if ($obj->song_id == 1 || $obj->device_id == 1)
